...
int main() {
    ...
    if (is_udp) {
        ...
        // sendto(socket_udp, &connection_type, sizeof(connection_type), 0,
        ...
        while (1) {
            ...
            if (strncmp(buf, "send", 4))
                ...
                send_command_udp(&server, socket_udp);
        }
    }
}
...
int find_server(struct sockaddr_in *server) {
    ...
    sendto(socket_udp, buf, buf_size, 0, (struct sockaddr *)&client,
           sizeof(client));

    socklen_t server_len = sizeof(*server);
    recvfrom(socket_udp, buf_receive, buf_size, 0, (struct sockaddr *)server,
             &server_len);
...
int authentication(struct sockaddr_in *server, char *user_name) {
    ...
    perror("socket");
    int reuse = 1;
    setsockopt(socket_tcp, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuse,
               sizeof(int));
    perror("setsocketopt");
...
int write_mess_udp(int socket_udp, mess_t message, char *buf,
                   struct sockaddr_in *server) {
    int ret = sendto(socket_udp, &message, sizeof(message), 0,
                     (struct sockaddr *)server, sizeof(*server));
    if (ret < 0)
        log_perror("write");
    ret = sendto(socket_udp, buf, message.mess_size, 0,
                 (struct sockaddr *)server, sizeof(*server));
    if (ret < 0)
        log_perror("write");
    return ret;
...
int send_file_udp(struct sockaddr_in *server, int socket_udp) {
    ...
    socklen_t server_len = sizeof(server);
    recvfrom(socket_udp, buf, BUF_SIZE, 0, (struct sockaddr *)&server,
             &server_len);
...
int get_file_udp(struct sockaddr_in *server, int socket_udp) {
    ...
    recvfrom(socket_udp, (char *)&file, sizeof(file), 0,
    ...
    mess_t path;
    recvfrom(socket_udp, (char *)&path, sizeof(path), 0,
             (struct sockaddr *)server, &server_len);
    ...
    if (recvfrom(socket_udp, buf_file, file.mess_size, 0,
                 (struct sockaddr *)server, &server_len) != file.mess_size)
        perror("recv message");
    if (recvfrom(socket_udp, path_name, path.mess_size, 0,
                 (struct sockaddr *)server, &server_len) != path.mess_size)
        perror("recv path");
    printf("path_name = %s\n", path_name);
...
int send_command_udp(struct sockaddr_in *server, int socket_udp) {
    ...
    sendto(socket_udp, buf, strlen(buf), 0, server, sizeof(*server));
    // mess_t answer;
    socklen_t server_len = sizeof(server);
    // recvfrom(socket_udp, (char*) &answer, sizeof(answer), 0, (struct
    ...
    recvfrom(socket_udp, (char *)mybuf, 1000, 0, (struct sockaddr *)server,
             &server_len);
    mybuf[strlen(mybuf)] = '\n';
